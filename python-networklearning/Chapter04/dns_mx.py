...
def resolve_hostname(hostname, indent=''):
    "Print an A or AAAA recored for `hostname`; follow CHAMEs if necessary."
    indent = indent + '    '
...
def resolve_email_domain(domain):
    ...
    if answer.rrset is not None:
        records = sorted(answer, key=lambda re: re.preference)
...
if __name__ == '__main__':
    parse = argparse.ArgumentParser(description='Find mailserver IP address')
    parse.add_argument('domain', help='domain that you want to send mail to')
    resolve_email_domain(parse.parse_args().domain)
